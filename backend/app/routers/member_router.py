...
@with_db_session
@verify_token
def create_member(db, user):
    try:
        member_data = MemberCreate(**request.json)
        member = member_service.create_member(db, user["id"], member_data)
...
@with_db_session
@verify_token
def get_member(db, user, member_id):
    member = member_service.get_member(db, user["id"], UUID(member_id))
    if not member:
...
@with_db_session
@verify_token
def get_all_members(db, user):
    skip = request.args.get("skip", 0, type=int)
    limit = request.args.get("limit", 100, type=int)
    members = member_service.get_all_members(db, user["id"], skip, limit)
...
@with_db_session
@verify_token
def update_member(db, user, member_id):
    try:
        member_data = MemberUpdate(**request.json)
        member = member_service.update_member(db, user["id"], UUID(member_id), member_data)
        if not member:
            return jsonify({"error": MEMBER_NOT_FOUND}), 404
        return jsonify(MemberResponse.model_validate(member).model_dump(mode="json")), 200
    except Exception as e:
...
@with_db_session
@verify_token
def delete_member(db, user, member_id):
    success = member_service.delete_member(db, user["id"], UUID(member_id))
    if not success:
        return jsonify({"error": MEMBER_NOT_FOUND}), 404
